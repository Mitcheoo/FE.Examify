...
export class ExamReadingComponent implements OnInit, OnDestroy {
  ...
  getAnsweredCount(start: number, end: number): number {
    if (!this.exam || !this.exam.questions) return 0;
    const questions = this.exam.questions.slice(start, end);
    return questions.filter((q: Question) => this.answers[q.id] && this.answers[q.id].trim()).length;
  }
