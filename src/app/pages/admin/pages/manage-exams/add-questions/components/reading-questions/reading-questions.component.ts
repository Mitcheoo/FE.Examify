// src/app/pages/admin/pages/manage-exams/add-questions/components/reading-questions/reading-questions.component.ts
...
export class ReadingQuestionsComponent implements OnInit {
  ...
  loadExistingQuestions(): void {
    if (this.questions && this.questions.length > 0) {
      // Load passages từ questions
      this.questions.forEach((q: any) => {
        if (q.partNumber && q.passage) {
          this.partPassages[q.partNumber] = q.passage;
        }
      });
    }
  }
  ...
  onPassageChange(partNumber: number): void {
    // Cập nhật passage cho các câu hỏi thuộc part đó
    this.questions.forEach((q: any) => {
      if (q.partNumber === partNumber) {
        q.passage = this.partPassages[partNumber];
      }
    });
  ...
  saveQuestion(): void {
    ...
    // Lấy passage từ partPassages
    const partNumber = this.newQuestion.partNumber;
    const passage = this.partPassages[partNumber] || '';
  ...
  getQuestionsByPart(partNumber: number): number {
    return this.questions.filter(q => q.partNumber === partNumber).length;
  }
}